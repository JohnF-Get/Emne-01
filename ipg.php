...
// Set the timezone.
date_default_timezone_set('Europe/Oslo'); // Replace with a relevant timezone
...
function generateSubdirectoryIndex(string $subdirectoryPath, string $relativePathToRoot, string $scriptFileName): void
{
  ...
  // Calculate relative path from the current subdirectory to its parent directory
  $relativePathFromSubdirectoryToParent = '../';
  ...
  $html = buildHTMLDocument($content, $relativePathFromSubdirectoryToParent, $shouldShowParentLink, $lang, $subdirectoryPath);
...
/**
 * Generates breadcrumb navigation for the current directory path.
 *
 ...
 */
function generateBreadcrumbs(string $rootDirectory, string $currentDirectory): string
{
  global $trans;

  // Get the relative path from the root directory to the current directory
  $relativePath = str_replace($rootDirectory, '', $currentDirectory);
  $pathParts = array_filter(explode(DIRECTORY_SEPARATOR, $relativePath));

  // Start breadcrumb HTML
  $breadcrumbHTML = '<nav class="breadcrumb">';
  $breadcrumbHTML .= '<span>' . $trans['breadcrumb_label'] . '</span>';

  // Root directory
  $breadcrumbHTML .= basename($rootDirectory);

  // Iterate through each directory level and add folder names
  foreach ($pathParts as $part) {
    $breadcrumbHTML .= getBreadcrumbArrowSVG();
    $breadcrumbHTML .= htmlspecialchars($part);
  }
...
/**
 * Returns the SVG for the right arrow icon used in breadcrumbs.
 */
function getBreadcrumbArrowSVG(): string
{
  return <<<SVG
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" stroke="#000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-arrow-right" style="margin: 0 8px;">
...
{
  global $trans, $rootDirectory;

  $breadcrumbs = generateBreadcrumbs($rootDirectory, getcwd());
  ...
  $githubLink = '<a href="https://github.com/JohnFilipstad/index-page-generator" target="_blank"><img src="https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png" alt="GitHub" style="width:16px;height:16px;vertical-align:middle;margin-right:5px;">Index Page Generator</a>';
  $generatedMessage = $subdirectoryPath
    ? basename($subdirectoryPath) . "/index.html " . $trans['generated_using'] . " $githubLink."
    : "index.html " . $trans['generated_using'] . "ipg_updated_message.php $githubLink.";
 ...
 <head>
     ...
     <meta name="description" content="Dette repositoriet er et levende arkiv av mine oppgaver knyttet til kurset. Det er viktig å merke seg at noen av filene her er uferdige og brukes kun til eksperimenteringsformål.">
 ...
 <body class="page">
     <header class="header" role="banner">
         ...
         <p class="code-repository"><a
        href="https://github.com/JohnF-Get/Emne-01"
        target="_blank"
        style="
      ...
      >
        <img
          ...
          style="
            ...
            margin-right: 8px;
          "
        />Repositoryet på GitHub
      </a></p>
     </header>
     $breadcrumbs
     <main role="main" class="content">
       <div>
         <h2>Levende arkiv av kursoppgavene mine</h2>
         <p>
          Dette repositoryet er et levende arkiv av mine oppgaver knyttet til
          kurset. Det er viktig å merke seg at noen av filene her er uferdige og
          brukes kun til eksperimenteringsformål.
         </p>
       </div>
       <div class="file-list">
        $parentLinkHTML
     ...
     <aside>
      <p><strong>Vær oppmerksom på følgende:</strong></p>
      <ul class="list">
        <li>Flere filer er under utvikling og kan inneholde uferdige eller foreløpige løsninger.</li>
        <li>Innholdet kan endres, ettersom jeg kontinuerlig jobber med å forbedre og utvide oppgavene i løpet av kurset.</li>
        <li>Dette arkivet er ment å være en ressurs for min læring og utvikling gjennom kurset, men det kan forekomme feil og uferdige løsninger.</li>
      </ul>
      <p>Jeg setter pris på tilbakemeldinger og innspill, men vær klar over at
          ikke alt innhold her nødvendigvis er endelig eller fullt dokumentert.</p>

      <p>Takk for at du viser interesse for kursoppgavene mine!</p>
     </aside>
     <footer role="contentinfo" class="footer">$generatedMessage</footer>
 </body>
