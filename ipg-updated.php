...
function generateBreadcrumbs(string $rootDirectory, string $currentDirectory): string
{
  ...
  // Root directory
  $breadcrumbHTML .= '<a href="./">' . basename($rootDirectory) . '</a>';

  // Iterate through each directory level
  $pathAccumulated = '';
  foreach ($pathParts as $part) {
    $pathAccumulated .= DIRECTORY_SEPARATOR . $part;
    $breadcrumbHTML .= getBreadcrumbArrowSVG();
    $breadcrumbHTML .= '<a href="' . htmlspecialchars($pathAccumulated) . '/">' . htmlspecialchars($part) . '</a>';
  }
...
function buildHTMLDocument(string $content, string $relativePathFromSubdirectoryToRoot, bool $shouldShowParentLink, string $lang, ?string $subdirectoryPath = null): string
{
  ...
  $generatedMessage = $subdirectoryPath
    ? basename($subdirectoryPath) . "/index.html " . $trans['generated_on'] . " $generatedTime " . $trans['using'] . " $githubLink."
    : "index.html " . $trans['generated_on'] . " $generatedTime " . $trans['using'] . " $githubLink.";
 ...
 <body class="page">
     ...
     <aside>
      <h3>Levende arkiv av kursoppgaver</h3>

        <p>
          Dette repositoriet er et levende arkiv av mine oppgaver knyttet til
